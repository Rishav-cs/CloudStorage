...
})({"cZxdl":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "1a60cdf609923b68";
"use strict";
...
},{}],"eVk6L":[function(require,module,exports,__globalThis) {
...
document.addEventListener('DOMContentLoaded', ()=>{
    const client = new (0, _appwrite.Client)().setEndpoint('https://fra.cloud.appwrite.io/v1').setProject('682355d400167120e0f2');
    const account = new (0, _appwrite.Account)(client);
    const storage = new (0, _appwrite.Storage)(client);
    const sidebarLinks = document.querySelectorAll('.sidebar a');
    const searchInput = document.getElementById('searchInput');
    const uploadBtn = document.getElementById('uploadBtn');
    const fileInput = document.getElementById('fileInput');
    ...
    const modalClose = document.getElementById('modalClose');
    const spinner = document.getElementById('spinner');
    const BUCKET_ID = '6824064400120ea49b27'; // Correct bucket ID
    let filesData = [];
    let currentTag = '';
    async function checkAuth() {
        try {
    ...
    async function fetchFiles() {
        ...
        try {
            ...
            console.error('Error fetching files:', error);
            if (error.code === 404) alert('Storage bucket not found. Please contact the administrator to verify the bucket ID.');
        } finally{
            spinner.classList.add('hidden');
        }
    }
    function updateStats() {
        ...
        const maxStorage = 2147483648; // 2GB in bytes
        const percent = totalSize / maxStorage * 100;
        // Update Available Storage tile
        storagePercent.textContent = `${percent.toFixed(2)}%`;
        storagePercent.parentElement.children[1].textContent = `${formatBytes(totalSize)} / 2GB`;
    ...
    function renderFiles() {
        ...
        filteredFiles.forEach((file)=>{
            ...
            const thumbnail = document.createElement('img');
            thumbnail.className = 'thumbnail';
            thumbnail.src = file.mimeType.startsWith('image/') ? `https://fra.cloud.appwrite.io/v1/storage/buckets/${BUCKET_ID}/files/${file.$id}/view?project=6824064400120ea49b27` : 'https://via.placeholder.com/40';
            const link = document.createElement('a');
            link.className = 'file-link';
            link.textContent = file.name;
            link.href = `https://fra.cloud.appwrite.io/v1/storage/buckets/${BUCKET_ID}/files/${file.$id}/view?project=6824064400120ea49b27`;
            link.target = '_blank';
            const actions = document.createElement('div');
            const infoBtn = document.createElement('button');
            infoBtn.textContent = 'Info';
            ...
            infoBtn.onclick = ()=>{
                if (modal && modalInfo) {
                    ...
                    `;
                    modal.classList.remove('hidden');
                } else console.error('Modal or modalInfo element not found');
            };
            downloadBtn.onclick = async ()=>{
                const url = `https://fra.cloud.appwrite.io/v1/storage/buckets/${BUCKET_ID}/files/${file.$id}/download?project=6824064400120ea49b27`;
                window.location.href = url;
            };
            deleteBtn.onclick = async ()=>{
                if (confirm('Are you sure you want to delete this file?')) {
    ...
    function initDragAndDrop() {
        ...
        document.body.ondrop = async (e)=>{
            ...
            spinner.classList.remove('hidden');
            try {
                for (const file of files)await storage.createFile(BUCKET_ID, (0, _appwrite.ID).unique(), file);
                await fetchFiles();
            } catch (error) {
                console.error('Error uploading files via drag and drop:', error);
            } finally{
    ...
    function initModal() {
        if (modalClose && modal) modalClose.onclick = ()=>{
            modal.classList.add('hidden');
        };
        else console.error('modalClose or modal element not found');
    }
    sidebarLinks.forEach((link)=>{
        link.onclick = (e)=>{
            e.preventDefault();
...
},{"appwrite":"gDEEN"}]},["cZxdl","eVk6L"], "eVk6L", "parcelRequirecf13")

//# sourceMappingURL=dashboard.09923b68.js.map
